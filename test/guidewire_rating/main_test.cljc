(ns guidewire_rating.main-test
  "Contract + behavioral test for the guidewire_rating-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "guidewire_rating-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'guidewire_rating.main-test)]
