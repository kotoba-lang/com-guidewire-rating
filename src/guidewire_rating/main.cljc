(ns guidewire_rating.main
  "Kotodama WASM entrypoint for the Guidewire_rating clean-room actor (L4) — Clojure port.
  ...
  facts (`guidewire_rating.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "guidewire_rating")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "guidewir_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}                  :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "guidewir_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"guidewire_rating.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "guidewire_rating-compat" :tier tier :entities entities} 200])
...
(defn start! [] :guidewire_rating-compat/ready)
